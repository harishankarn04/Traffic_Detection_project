...
- [2026-05-19] Migrated `demo_runner.py` and `batch_video_comparison.py` to use Ultralytics YOLO engine (resolving manual ONNX parsing issues). Finalized universal 8-class mapping system (`car`, `bus`, `truck`, `motorcycle`, `auto_rickshaw`, `ambulance`, `fire_truck`, `van`) including fix for `van`/`cng`/`tempo` aliases. Updated `auto_train_windows.py` to preserve weights and export metrics graphs.
